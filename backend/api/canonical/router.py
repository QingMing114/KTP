...
def build_canonical_router() -> APIRouter:
    ...
    _HTML_ARTIFACT_KINDS: set[str] = {"lai_html_report"}
    ...
    _idem_cache: dict[str, tuple[str, bytes, JSONResponse]] = {}
    """idempotency_key → (path_template, body_hash, cached_response)

    .. warning::
        In-memory only — a service restart invalidates all cached keys.
        Clients should not reuse ``Idempotency-Key`` across restarts.
        A future change should persist the cache to SQLite."""
    ...
    def _idempotency_result(
        ...
        """Check idempotency and cache the result.

        Returns a cached 409 conflict response, or None to proceed.
        The caller is responsible for caching the success response after
        the request completes, by calling again with the same key and the
        final response.  Since we don't want to cache before processing,
        we use a two-phase approach: check before, cache after.
        """
        ...
        if idempotency_key in _idem_cache:
            cached_path, cached_hash, cached_resp = _idem_cache[idempotency_key]
            if cached_path != path_template:
                return canonical_error_response(
                    409,
                    code=ErrorCode.IDEMPOTENCY_CONFLICT,
                    message=f"Idempotency-Key already used for a different endpoint",
                    request_id=_req_id(),
                )
            ...
            logger.info("idempotency_replay | key=%s", idempotency_key[:8])
            return cached_resp
    ...
    def _cache_idempotency(
        ...
        if idempotency_key is None:
            return
        body_hash = hashlib.sha256(body_bytes).digest()
        _idem_cache[idempotency_key] = (path_template, body_hash, response)
    ...
    def _inherit_context(store: RuntimeStore, conversation_id: str, context: dict | None) -> RequestContextV2:
        ...
        """
        if context and context.get("inherit") == "latest":
            runs = store.list_runs_for_session(conversation_id)
            if runs:
                sorted_runs = sorted(runs, key=lambda r: r.created_at or "")
                latest = sorted_runs[-1]  # most recent
    ...
    async def get_manifest(request: Request) -> ManifestResponse:
        ...
        return ManifestResponse(
            ...
            delivery_modes=["async", "sync"],
            supported_preferences=["include_visualization", "include_knowledge"],
            artifact_kinds=["lai_html_report", "inference_card", "report_card", "visualization"],
    ...
    async def create_lai_analysis(body: CreateLaiAnalysisRequest, request: Request) -> LaiAnalysisSummary:
        ...
        def event(kind: str, stage: str, detail: str, progress: int | None = None, data: dict | None = None) -> None:
            ...
            analyses.update(analysis.analysis_id, **update_fields)
            analyses.push_event(analysis.analysis_id, {
                "event_id": str(uuid4()),
        ...
        def worker() -> None:
            try:
                ...
                item = imagery_provider.get_item(body.imagery_item_id)
                event("progress", "imagery", "影像元数据已就绪，正在解析波段资源", 8)
                work_dir = Path(__file__).resolve().parents[2] / "var" / "artifacts" / "lai" / analysis.analysis_id
                ...
                if not isinstance(report_path_value, str) or not Path(report_path_value).is_file():
                    raise RuntimeError("LAI 报告生成后未能通过文件校验")
                ...
                if report_artifact_id is None:
                    raise RuntimeError("LAI 报告生成后未注册为可查看产物")
                ...
                payload["report_uri"] = f"/api/product/v1/artifacts/{report_artifact_id}/render"
                analyses.update(analysis.analysis_id, status="completed", stage="completed", completed_at=datetime.now(timezone.utc).isoformat(), artifact_ids=[reflectance_id, *registered], result=payload)
                event("result", "completed", "LAI 反演和报告已完成", 100, {"artifact_ids": [reflectance_id, *registered], **payload})
                ...
                analyses.update(analysis.analysis_id, status="failed", stage="failed", completed_at=datetime.now(timezone.utc).isoformat(), result={"error": detail})
                event("error", "failed", detail)
    ...
    async def stream_lai_analysis_events(analysis_id: str, request: Request) -> StreamingResponse:
        queue = _lai_analysis_store(request).queue(analysis_id)
        if queue is None:
            raise HTTPException(status_code=404, detail="LAI analysis event stream unavailable")
    ...
    async def create_dataset(
        ...
        # Idempotency
        idem_resp = _idempotency_result(idempotency_key, "/datasets", body_bytes)
        if idem_resp is not None:
            return idem_resp

        # Idempotency replay via dataset store
        if idempotency_key:
            existing_id = ds_store.check_idempotency_conflict(idempotency_key)
            if existing_id:
                existing = ds_store.get_dataset(existing_id)
        ...
        _cache_idempotency(idempotency_key, "/datasets", body_bytes,
                           JSONResponse(status_code=201, content=resp_body))
        return dataset
    ...
    async def create_conversation(
        ...
        idem_resp = _idempotency_result(idempotency_key, "/conversations", body_bytes)
        if idem_resp is not None:
            return idem_resp
        ...
        _cache_idempotency(idempotency_key, "/conversations", body_bytes,
                           JSONResponse(status_code=201, content=result.model_dump(mode="json")))
        return result
    ...
    async def create_submission(
        ...
        # Idempotency
        idem_resp = _idempotency_result(idempotency_key, "/conversations/{conversation_id}/submissions", body_bytes)
        if idem_resp is not None:
            return idem_resp
        ...
        mode = body.mode or {}
        delivery = mode.get("delivery", "async")
        if delivery != "async":
            return canonical_error_response(
        ...
        interaction = mode.get("interaction", "chat")

        # Extract message and refs
        user_message = body.input.get("message", "") if isinstance(body.input, dict) else str(body.input)
        refs = body.input.get("refs", []) if isinstance(body.input, dict) else []

        # Validate dataset refs exist
        for ref in refs:
            if isinstance(ref, dict) and ref.get("type") == "dataset":
                ...
                if ds is None:
                    return canonical_error_response(
                        ...
                        request_id=_req_id())
                user_message += f" [dataset: {ds.display_name} at {ds.source.uri}]"
        ...
        def worker() -> None:
            try:
                # queued → running
                sub_store.update_submission(submission_id, status="running", stage="processing")
                ...
                for internal_event in engine.stream(
                    ...
                    if internal_event.event in ("run_completed", "run_failed"):
                    ...
                    if internal_event.event == "run_completed":
                        # Register artifacts into stable store before marking completion
                        run = store.get_run(internal_event.run_id)
                        if run is not None and run.artifacts:
                            for artifact in run.artifacts:
                                art_store.register(run_id=internal_event.run_id, artifact=artifact)
                        sub_store.update_submission(submission_id, status="completed", stage="completed",
                                                    completed_at=datetime.now(timezone.utc).isoformat())
                        _emit_event(sub_store, submission_id, "run.completed", stage="completed",
                                     run_id=internal_event.run_id)
                    elif internal_event.event == "run_failed":
                        sub_store.update_submission(submission_id, status="failed", stage="failed",
                                                    completed_at=datetime.now(timezone.utc).isoformat())
        ...
        _cache_idempotency(idempotency_key, "/conversations/{conversation_id}/submissions", body_bytes,
                           JSONResponse(status_code=201, content=result.model_dump(mode="json")))
        return result
    ...
    async def stream_submission_events(
        ...
        queue = sub_store.get_event_queue(submission_id)
        if queue is None:
            return canonical_error_response(
        ...
        async def event_generator() -> AsyncIterator[str]:
            ...
            # Reconnection: send snapshot first
            if last_seq > 0:
                snapshot = sub_store.get_submission(submission_id)
                if snapshot is not None:
                    seq += 1
                    snapshot_data = snapshot.model_dump(mode="json")
                    yield (
                        f"id: evt_{seq:04d}\n"
                        f"event: submission.updated\n"
            ...
            while True:
                ...
                payload = {k: v for k, v in event_data.items() if k not in ("event",)}
    ...
    async def cancel_submission(
        ...
        idem_resp = _idempotency_result(idempotency_key, "/submissions/{submission_id}/cancel", body_bytes)
        if idem_resp is not None:
            return idem_resp
        ...
        _emit_event(sub_store, submission_id, "submission.cancelled", stage="failed", detail="Cancelled by user")
        sub_store.update_submission(submission_id, status="cancelled", stage="failed",
                                    completed_at=datetime.now(timezone.utc).isoformat())
        sub_store.close_submission(submission_id)

        updated = sub_store.get_submission(submission_id)
        result = updated if updated else submission
        _cache_idempotency(idempotency_key, "/submissions/{submission_id}/cancel", body_bytes,
                           JSONResponse(status_code=200, content=result.model_dump(mode="json")))
        return result
...
def install_canonical_product_api(app, db_path: str | None = None) -> APIRouter:
    ...
    Initializes ``product_submission_store``, ``product_dataset_store`` and
    ``product_artifact_store`` on ``app.state`` at install time (not lazily
    on first request), avoiding race conditions under concurrent startup.
    ...
    global _installed
    if _installed:
        logger.debug("canonical_product_api_already_installed — skipping duplicate")
        return APIRouter()
    ...
    app.state.product_submission_store = SubmissionStore(db_path=db_path, loop=asyncio.get_event_loop())
    ...
    app.state.lai_analysis_store = LaiAnalysisStore(db_path=db_path, loop=asyncio.get_event_loop())
